...
\begin{column}{0.55\textwidth}
    \begin{block}{\faCloud\ Resource Group: bigData}
        \footnotesize
        \begin{tabular}{ll}
            \toprule
            \textbf{Ressource} & \textbf{Détails} \\
            \midrule
            VM-Master & Standard\_B2s \\
            VM-Worker-1 & Standard\_B2s \\
    ...
    \begin{alertblock}{\faExclamationTriangle\ fraud-detection-rg}
        \footnotesize
        \begin{itemize}
            \item Région: West Europe
            \item Créé via Azure CLI
            \item Pour Databricks deployment
        \end{itemize}
    \end{alertblock}
\end{column}
\begin{column}{0.43\textwidth}
    \begin{exampleblock}{\faCheckCircle\ Configuration Cluster}
        \footnotesize
        \begin{itemize}
            \item \textbf{Architecture:} Master/Worker
            \item \textbf{Cores:} 2 vCPUs par VM
            \item \textbf{RAM:} 4 GB par VM
            \item \textbf{Storage:} 30 GB SSD
            \item \textbf{Network:} VNet privé
        \end{itemize}
    \end{exampleblock}
    
    \begin{block}{\faDollarSign\ Azure for Students}
        \footnotesize
        \begin{itemize}
            \item Crédit: \$100
            \item Durée: 12 mois
            \item VMs éligibles: B-series
        \end{itemize}
    \end{block}
\end{column}
\end{columns}
...
\begin{frame}[fragile]{Azure CLI - Création des Ressources}
\begin{columns}[T]
\begin{column}{0.55\textwidth}
    \begin{block}{\faTerminal\ Commandes Exécutées}
...
\end{lstlisting}
    ...
    \begin{exampleblock}{\faCheckCircle\ Réponse JSON}
...
\end{column}
\begin{column}{0.43\textwidth}
    \begin{block}{\faList\ Étapes Déploiement}
        \footnotesize
        \begin{enumerate}
            \item Connexion Azure CLI
            \item Création Resource Group
            \item Déploiement ARM Template
            \item Configuration Databricks
            \item Upload des données
        \end{enumerate}
    \end{block}
    
    \begin{alertblock}{\faRocket\ Status}
        \footnotesize
        \begin{itemize}
            \item[\faCheck] Resource Group créé
            \item[\faCheck] Subscription active
            \item[\faCheck] Région disponible
            \item[\faClock] Databricks: En attente
        \end{itemize}
    \end{alertblock}
\end{column}
\end{columns}
