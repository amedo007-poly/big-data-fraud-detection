...
\begin{center}
    \fbox{\parbox{0.9\textwidth}{
        \centering
        \textbf{AZURE PORTAL - RESOURCE GROUPS}\\[0.5cm]
        \texttt{bigData} Resource Group:\\
        VM-Master + VM-Worker-1 (Switzerland North)\\[0.3cm]
        \texttt{fraud-detection-rg} (West Europe)\\
        Créé via Azure CLI
    }}
\end{center}
\footnotesize
\begin{itemize}
    \item Cluster Spark: 1 Master + 1 Worker
    \item Région: Switzerland North (Azure for Students)
\end{itemize}
\end{frame}
...
\begin{center}
    \begin{tikzpicture}
        \node[draw=darkblue, fill=gray!10, rounded corners, text width=10cm, font=\ttfamily\tiny] {
            \$ az login\\
            Tenant: GFI\\
            Subscription: Azure for Students\\[0.2cm]
            \$ az group create --name fraud-detection-rg --location westeurope\\[0.1cm]
            \{\\
            \quad "id": "/subscriptions/.../resourceGroups/fraud-detection-rg",\\
            \quad "location": "westeurope",\\
            \quad "provisioningState": "Succeeded"\\
            \}
        };
    \end{tikzpicture}
\end{center}
\footnotesize
Resource Group créé avec succès pour le déploiement Databricks
\end{frame}
